...
/*   Updated: 2025/12/26 19:47:45 by moaatik          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_dda(t_game *game)
{
	...
	if (game->ray.dir_x < 0)
		game->cast.side_dist_x = ((game->player.x / BLOCK_SIZE)
				- game->cast.map_x) * game->cast.step_x;
	else
		game->cast.side_dist_x = ((game->cast.map_x + 1.0f)
				- (game->player.x / BLOCK_SIZE)) * game->cast.step_x;
	if (game->ray.dir_y < 0)
		game->cast.side_dist_y = ((game->player.y / BLOCK_SIZE)
				- game->cast.map_y) * game->cast.step_y;
	else
		game->cast.side_dist_y = ((game->cast.map_y + 1.0f)
				- (game->player.y / BLOCK_SIZE)) * game->cast.step_y;
}
...
float	distance(t_game *game)
{
	...
	wall_dist *= game->player.dir_x * game->ray.dir_x
		+ game->player.dir_y * game->ray.dir_y;
	return (wall_dist);
}
